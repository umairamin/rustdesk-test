...
class Dialog<T> {
  ...
  void complete(T? res) {
    try {
      ...
      print("Dialog complete catch error: $e");
    } finally {
      entry?.remove();
    }
  }
...
Future<void> initGlobalFFI() async {
  print("_globalFFI init");
  _globalFFI = FFI(null);
  print("_globalFFI init end");
...
Future<void> saveWindowPosition(WindowType type, {int? windowId}) async {
  ...
  if (isWindows) {
    ...
    if (position.dx < kMinOffset ||
      ...
      print("Invalid position: $position, ignore saving position");
      return;
    }
  }
...
Future<bool> restoreWindowPosition(WindowType type,
  ...
  var lpos = LastWindowPosition.loadFromString(pos);
  if (lpos == null) {
    print("no window position saved, ignoring position restoration");
    return false;
  }
  if (type == WindowType.RemoteDesktop || type == WindowType.ViewCamera) {
...
StreamSubscription? listenUniLinks({handleByFlutter = true}) {
  ...
  final sub = uriLinkStream.listen((Uri? uri) {
    print("A uri was received: $uri. handleByFlutter $handleByFlutter");
    if (uri != null) {
      if (handleByFlutter) {
...
Future<List<Rect>> getScreenListWayland() async {
  ...
  if (isMainDesktopWindow) {
    ...
    try {
      ...
      print('Failed to parse screenList: $e');
    }
  }
...
Future<List<Rect>> getScreenListNotWayland() async {
  ...
  try {
    ...
    print('Failed to parse displays: $e');
  }
  return screenRectList;
}
...
List<String> getPrinterNames() {
  ...
  try {
    ...
    print('failed to parse printer names, err: $e');
    return [];
  }
}
